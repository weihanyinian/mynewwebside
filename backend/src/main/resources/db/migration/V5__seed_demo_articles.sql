...
SELECT
  ...
  CONCAT(
    ...
    '```java\n',
    '@ConditionalOnProperty(name = "app.view-counter.redis", havingValue = "true")\n',
    '@Component\n',
    'public class RedisArticleViewCounter implements ArticleViewCounter {\n',
    '    // Redis INCR + 定时批量刷新到 MySQL\n',
    '}\n\n',
    '@ConditionalOnMissingBean(RedisArticleViewCounter.class)\n',
    '@Component\n',
    ...
    '```typescript\n',
    'const routes = [\n',
    '  { path: "/",            component: PortfolioPage },  // 首页 — 独立布局\n',
    '  { path: "/blog",        component: BlogHome },       // 博客列表\n',
    '  { path: "/article/:id", component: ArticlePage },    // 文章详情\n',
    '  { path: "/music",       component: MusicCenterPage, meta: { requiresAuth: true } },\n',
    '  { path: "/moyu/*",      children: [...] },           // 摸鱼小游戏\n',
    '  { path: "/admin/*",     component: AdminLayout },     // 后台管理\n',
    ']\n',
    '```\n\n',
    '三种布局模式：\n',
    '- **首页**：独立全屏设计，视频背景 + 玻璃拟态\n',
    ...
    '### 2. 状态管理设计（Pinia）\n\n',
    '```typescript\n',
    '// musicPlayer store — 音乐播放器全局状态\n',
    'useMusicPlayerStore: {\n',
    '  queue, currentIndex, resolvedUrl,  // 播放状态\n',
    '  neteaseBound, qqBound,              // 绑定状态\n',
    '  playMode, volume,                   // 用户偏好\n',
    '  initPlayer(), loadSong(), playNext() // 操作\n',
    '}\n\n',
    '// theme store — 主题切换\n',
    'useThemeStore: {\n',
    '  isDarkMode, followSystem,           // 状态\n',
    '  toggleTheme(), syncDocumentTheme()  // 操作\n',
    '}\n',
    '```\n\n',
    '### 3. 国际化（i18n）\n\n',
    ...
    '### 整体架构\n\n',
    '```\n',
    '浏览器 ─→ Spring Boot :8080/api/music/* ─→ NeteaseCloudMusicApi :3000 ─→ 网易云官方 API\n',
    '                                      ─→ QQMusicApi :3300           ─→ QQ 音乐 API\n',
    '```\n\n',
    '为什么要做一层代理而不是前端直接调？\n',
    '1. **Cookie 安全**：网易云的登录 Cookie 存储在服务端 Session/DB，不暴露给浏览器\n',
    '2. **跨域问题**：第三方 API 不设置 CORS 头，浏览器直调会失败\n',
    '3. **统一鉴权**：前端只需本站 JWT，后端判断是否有权限使用音乐功能\n',
    '4. **日志与监控**：所有第三方调用经过后端，方便排查问题\n\n',
    ...
    '        for (int i = 0; i <= props.getRetryCount(); i++) {\n',
    '            try {\n',
    '                return restTemplate.getForObject(url, type);\n',
    '            } catch (Exception e) {\n',
    '                if (i == props.getRetryCount()) throw e;\n',
    '                // 本地不可达时回退到公网镜像\n',
    '            }\n',
    '        }\n',
    '    }\n',
    ...
    '```\n\n',
    '### 双源热歌策略\n\n',
    '默认歌单每日轮换：奇数日优先网易云热歌，偶数日优先 QQ 热歌。一方失败时自动回退到另一方，都失败时回退到配置的固定歌单。\n\n',
    '## 性能优化实践\n\n',
    '### 前端\n',
    ...
    'services:\n',
    '  mysql:      # MySQL 9.1\n',
    '  redis:      # Redis 7（可选，浏览量计数用）\n',
    '  backend:    # Spring Boot JAR\n',
    '  frontend:   # Nginx + Vue 构建产物\n',
    '  ncm-api:    # NeteaseCloudMusicApi Enhanced（可选）\n',
    '  qq-api:     # QQMusicApi（可选）\n',
    '```\n\n',
    '使用 `docker compose up -d` 即可启动全部服务。支持通过 `.env` 文件配置各服务参数。\n\n',
    '## 总结与展望\n\n',
    '这个项目让我对全栈开发有了更系统的理解：\n\n',
    '1. **分层不是教条**：小型个人项目不需要过度设计，但 Controller-Service-Repository 三层让后续维护轻松很多\n',
    '2. **数据库迁移是底线**：Flyway 让任何环境（开发/测试/生产）的数据库状态可追溯、可复现\n',
    '3. **前端状态管理要克制**：不是所有状态都要进 Pinia——斗地主游戏全程用组件内 `reactive()` 管理，简单高效\n',
    '4. **代理层有价值**：音乐 API 的代理设计既解决了安全（Cookie 不暴露），也解决了跨域问题\n',
    '5. **好体验在细节**：自动跳过无版权歌曲、日夜间主题跟随系统、响应式到 320px 宽度——这些小事比大功能更重要\n\n',
    '后续计划：\n',
    '- 评论系统接入（目前留言墙已就位）\n',
